...
model_fit_pars <- data.frame(stan_model=rep(sort(rep(model_names, n_chains)),
                             ...
                             model_ID=rep(sort(rep(1:n_models, n_chains)),
                                      length(ModelData2)),
                             DatasetID=sort(rep(1:length(ModelData2), 
                                                n_chains*n_models)),
...
clusterExport(cl, varlist=c("model_fit_pars", "ModelData2"))
...
comp_mods <- parLapply(cl, model_names,  function(x){
  ...
  # Compile model
  stan(x, data=ModelData2[[1]], chains=1, iter=1)
})
...
sampled_chains <- parLapply(cl, 1:nrow(model_fit_pars), function(x){
  ...
  # Retrieve data
  md <- ModelData2[[model_fit_pars$DatasetID[x]]]
...
Model_List <- parLapply(cl, datamodels, function(i){
  ...
  dataName <- ModelData2[[dataID]]$model_name
...
# Save output of LOO
save(LOO_comp, file="02_m2_Out_LOO.RData")

# Save Models List in chunks
i1 <- seq(1, 101, 2)[seq(1, 101, 2) %in% 1:length(ModelData2)]
i2 <- seq(2, 102, 2)[seq(2, 102, 2) %in% 1:length(ModelData2)]
if(length(i1)>length(i2)) i2 <- c(i2, i1[length(i1)])
for(i in 1:ceiling(length(ModelData2)/2)){
  
  # Surbset models
  output_models <- Model_List[i1[i]:i2[i]]
  
  out_file <- paste0("02_m2_Out_Models_0", i, ".RData")
